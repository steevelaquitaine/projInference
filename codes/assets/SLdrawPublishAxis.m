...
%
%        $Id:$ 
%      usage: SLdrawPublishAxis()
%         by: justin gardner
% updated by: steeve laquitaine
%       date: 06/10/13
...
%
%To DO: 
% - Yaxis exponents are removed...solve that
%

function retval = SLdrawPublishAxis(varargin)
...
getArgs(varargin,{'whichAxis=both','tickDir=out','lineWidth=1','titleStr=[]','labelFontSize=12',...
		  ...
		  'yMinorTick=[]','yLabel=[]','yLabelOffset=-6/64','yTickLabelSigfigs=-1',...
		 });
...
if ishandle(dpa.titleHandle)
  ...
  %get title interpreter (e.g., tex or latex if math symbols)
  dpa.titleInterp = get(dpa.titleHandle,'interpreter');
...
if any(strcmp(whichAxis,{'both','horizonal'}))
  ...
  if (strcmp(xAxisLoc,'bottom') && strcmp(yDir,'normal')) || (strcmp(xAxisLoc,'top') && strcmp(yDir,'reverse'))
    ...
    if strcmp(tickDir,'in') tickDir = -1; else tickDir = 1; end
  else
  ...
  if (xAxisMajorTickLen < 0)
    xAxisTickYpos = logScaleSum(yScale,xAxisYpos,tickDir*xAxisMajorTickLen,abs(yMax-yMin));
  else
    xAxisTickYpos = xAxisYpos + tickDir * xAxisMajorTickLen;
  end

  % get the bottom position of the minor tick (xAxisMinorTickYpos)
  if (xAxisMinorTickLen < 0)
    xAxisMinorTickYpos = logScaleSum(yScale,xAxisYpos,tickDir*xAxisMinorTickLen,abs(yMax-yMin));
  else
    xAxisMinorTickYpos = xAxisYpos + tickDir * xAxisMinorTickLen;
  end
  ...
  if xAxisMinMaxSetByTicks, xAxisMin = min(xTick); xAxisMax = max(xTick); end

  % get the xLabelPosition
  xLabelX = logScaleMean(xScale,[xAxisMin xAxisMax]);
  xLabelY = logScaleSum(yScale,xAxisYpos,tickDir*xLabelOffset,abs(yMax-yMin));
end
...
if any(strcmp(whichAxis,{'both','vertical'}))
  ...
  if (yAxisMajorTickLen < 0)
    yAxisTickXpos = logScaleSum(xScale,yAxisXpos,tickDir*yAxisMajorTickLen,abs(xMax-xMin));
  else
    yAxisTickXpos = yAxisXpos+tickDir*yAxisMajorTickLen;
  end

  % get the left position of the minor tick (yAxisMinorTickXpos)
  if (yAxisMinorTickLen < 0)
    yAxisMinorTickXpos = logScaleSum(xScale,yAxisXpos,tickDir*yAxisMinorTickLen,abs(xMax-xMin));
  else
    yAxisMinorTickXpos = yAxisXpos + tickDir * yAxisMinorTickLen;
  end
  ...
  if yAxisMinMaxSetByTicks, yAxisMin = min(yTick); yAxisMax = max(yTick); end

  % get the xLabelPosition
  yLabelX = logScaleSum(xScale,yAxisXpos,tickDir*yLabelOffset,abs(xMax-xMin));
...
if ~isempty(titleStr)
  dpa.axis.title = text(titleX,titleY,titleStr,'HorizontalAlignment','center','VerticalAlignment',titleAlignment,'FontSize',labelFontSize,'FontAngle','oblique');
  
  %set back title interpreter (SL)
  set(dpa.axis.title,'interpreter',dpa.titleInterp)
end
...
if any(strcmp(whichAxis,{'both','horizonal'}))
  ...
  % set the text label
  if ~isempty(xLabel)
    dpa.xAxis.label = text(xLabelX,xLabelY,xLabel,'VerticalAlignment',xTextAlignment,'HorizontalAlignment','center','FontAngle','oblique','FontSize',labelFontSize);
  end
...
if any(strcmp(whichAxis,{'both','vertical'}))
  ...
  % set the text label
  if ~isempty(xLabel)
    dpa.yAxis.label = text(yLabelX,yLabelY,yLabel,'VerticalAlignment',yLabelTextAlignment,'HorizontalAlignment','center','FontAngle','oblique','FontSize',labelFontSize,'Rotation',90);
  end
