% slImportSurfaceOFF.m
%
%      usage: v = slImportSurfaceOFF
%         by: justin gardner modified by steeve laquitaine
%       date: 10/24/07 (150911)
%    purpose: Import a pair of inner and outer cortical surfaces in OFF format
%
%    base = slimportSurfaceOFF('/path/to/surfaces/subject_left_WM.off','bothHemiFlag','/path/to/canonical/SID.nii');
%    viewSet(getMLRView, 'newbase', base);

function base = slImportSurfaceOFF(pathStr,bothHemiFlag,pathCanon)

% check arguments
if ~any(nargin == [0 1 2 3])
  help gruImportSurfaceOFF
  return
end
base = [];
...
params1 = mrSurfViewer(filename,[],[],[],[],pathCanon);
...
if bothHemiFlag
  ...
  if leftFlag
    prefix = params1.outerSurface(1:leftFlag-1);
    postfix = params1.outerSurface(leftFlag+4:end);
    rightFilename = sprintf('%sright%s', prefix, postfix);
    params2 = mrSurfViewer(rightFilename,[],[],[],[],pathCanon);  
  end
  % or was it a right hemispehre
  rightFlag = strfind(lower(params1.innerSurface), 'right');
  if rightFlag
    prefix  = params1.outerSurface(1:rightFlag-1);
    postfix = params1.outerSurface(rightFlag+5:end);
    leftFilename = sprintf('%sleft%s', prefix, postfix);
    params2 = mrSurfViewer(leftFilename,[],[],[],[],pathCanon);
  end
...
surf.filename = sprintf('%s_%s', surf1.filename, surf2.filename);
