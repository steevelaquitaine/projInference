...
%                       set random values for missing classes


function slfmriClassSensStckSess2(o,myClass,varargin)
...
elseif slIsInput(varargin,'loadSavedInstances')
    ...
    if isempty(l)
        slPrintfStr('slfmriClassSensStckSess',...
            [' ClassifStckSessData' o.myClass '_allRois_..not found...'])
        keyboard
    else
        load(l.name)
    end
...
for roii = 1 : o.nRois
    ...
    c.myClasf.raw.fullSgShuf = leaveOneOutNpermut(Aligni{roii},'permutationUnBal=1','nPerm=50');
    c.myClasf.Zsc.fullSgShuf = leaveOneOutNpermut(Aligni{roii},'permutationUnBal=1','nPerm=50');
...
fprintf('%s \n','(slGetSessStimvol) Task conditions are: ')
...
mkdir([o.stckPath 'slStckAnalyses/' o.myGroup '/classif/' o.myClass '/' [o.myAnalysis{:}]]);
%save file
filename = ['ClassifStckSessData' o.myClass '_allRois_' date '.mat'];
save(filename,'o','c','s')
