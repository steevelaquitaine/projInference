...
%slfmriGetVoxTuningParamsVMfit.m
%
%
...
%
%       [o,prep,VoxelsParams] = slfmriGetVoxTuningParamsVMfit('myRandomDir',{'mySwitch=1','mySwitch=2'},'myRandomCoh=0.06',85,o,'useinputdb',d_dirBalbySw{coh_u==coherence},'getVoxTuning','vmfitr2cutoff=0','fminsearch');
%
%
%inputs:
...
%varargin:
%
%       "fminsearch"/"gridsearchMean": tuning parameter search type (mandatory)
...
function [o,prep,VoxelsParams,voxTunings] = slfmriGetVoxTuningParamsVMfit(var0,var1,var2,thisvar0,o,varargin)


%preprocess
prep = getPreprocessing(varargin{:});

%get tuning parameter search
search = getSearch(varargin{:});

%load instance database
cd(o.dataPath)
d = slfmriLoadInstancedb(varargin{1}{:});

%--------------- Voxels tuning -----------------
%when we need to get voxels tuning (takes some time)
%more for many voxels
if any(strcmp(varargin{1},'getVoxTuning'))
    fprintf('%s \n','(slfmriGetVoxTuningParamsVMfit) Getting voxel tunings.')
    ...
    if length(unique(d.(var0)))<3
        fprintf('%s \n','(slfmriGetVoxTuningParamsVMfit) Stopped. Not enough sample in x space (n<3) to fit von Mises to data. Fitting will produce meaningless results.')
        keyboard        
    end   
    ...
    if ~isempty(var1{1})
        parfor voxnum = 1 : nVox
            fprintf('-')
            st1 = slfmrigetVoxTuning(voxnum,var0,var1{1},var2,d,search);            
    ...
    if ~isempty(var1{2})
        parfor  voxnum = 1 : nVox
            fprintf('-')
            st2 = slfmrigetVoxTuning(voxnum,var0,var1{2},var2,d,search);            
...
%get tuning parameter search
function search = getSearch(varargin)

if any(strcmp(varargin{1},'gridsearchMean'))
    search = 'gridsearchMean';
elseif any(strcmp(varargin{1},'fminsearch'))
    search = 'fminsearch';
else
    fprintf('%s /n','you need to specify the search: "fminsearch", or "gridsearchMean"')
    keyboard
end
