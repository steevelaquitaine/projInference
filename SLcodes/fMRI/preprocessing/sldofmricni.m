...
%
function retval = sldofmricni(varargin)
...
if ~s.useLocalData
  ...
  elseif s.terminalInput==1
      
      %ask user to write down remote stimfiles path in the terminal
      myStimInputDir = input('input stimfiles path on remote computer (e.g.~/data/sltaskDotDirfmri05/s0025)','s');
      ...
      while isempty(stimfileListing)
          ...
          paramsInfo{end+1} = {'computerUserName',s.stimComputerUserName,'Name of user on computer for ssh login'};
          paramsInfo{end+1} = {'stimfileStem',s.stimfileStem,'The base of the stimfile names'};
          paramsInfo{end+1} = {'dataDir',myStimInputDir,'Name of directory on %s where stimfiles are located'};
