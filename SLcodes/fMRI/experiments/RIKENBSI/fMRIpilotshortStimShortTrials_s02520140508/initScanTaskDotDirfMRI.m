   ...
   %author: steeve
     %date: 140508
  %purpose: make time series of displayed directions for different 
            %coherences conditions
    %usage: %[task]=initScanTaskDotDirfMRI('steeve_fMRIpilotllhShortStimShorTrials_metho_uniform_coh012100_dir6_t30perCoh_TR157_140508',225,inf,[.12 1],[30 30],[345 45 105 165 225 285],[.5 .5 .5])
    
function task=initScanTaskDotDirfMRI(Prname,...
    Prmean,Prstd,coh,Prnumtrials_perCoh,PrSp,colorH)
...
for i=1:numCoh
    pos=i+tmp(i);
    subplot(numCoh,2,pos);
    title([num2str(coh),' coherence'])
    SLcircHist(task.parameter.dir.sample.degree,task.parameter.dir.countperCoh(i,:),colorH,5,'text')
end
...
for i=1:numCoh
    ...
    text(0.9*max(task.parameter.dir.sample.degree),.90*max(task.parameter.dir.countperCoh(1,:)),strcat(num2str(coh(i)*100),'% coh'),'fontsize',14)
    drawPublishAxis
end
...
bar(1:numel(sample)+1,[count count(1)],...
    'FaceColor',colorH,...
    'BarWidth',myBarWidth,...
    'EdgeColor','none')
set(gca,'xtick',1:numel(sample)+1,'xticklabel',[sample bar_xend])
%lines
