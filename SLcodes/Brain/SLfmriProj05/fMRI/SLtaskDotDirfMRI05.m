...
%   purpose: random dot motion direction estimation task for fMRI. Identify 
%            brain areas that support mechanims of switching between prior 
%            and direction evidence.
%
%`    usage: 
%
%           mglSimulateRun(.5,630,0,'a')
%           sltaskDotDirfMRI05('fMRIprojFlex','fMRIParamsPrior80Coh12and06_block01',1,3)
%
%   details:  
%    
...
function myscreen = SLtaskDotDirfMRI05(screenName,paramsFile,RespConfirm,RespType)
...
%#vols : 46 per trials (= 23s trial/0.5s tr)
%[Fix - motion - ISI - Response - ITI]
%task{2}{1}.segmin   = [1 .3 5 5 11.7];
%task{2}{1}.segmax   = [1 .3 5 5 11.7];
task{2}{1}.segmin   = [1 .3 5 8 11.7];
task{2}{1}.segmax   = [1 .3 5 8 11.7];
...
%adjust coherence
%----------------
params.task.parameter.dir.coh(params.task.parameter.dir.coh==0.12)=0.16;
params.task.parameter.dir.coh(params.task.parameter.dir.coh==0.06)=0.08;


%100% coh
%task{2}{1}.randVars.myRandomCoh = ones(1,length(myTrialRand));
...
elseif myscreen.RespConfirm == 3
    task{2}{1}.getResponse = [0 0 0 1 0];
    fprintf('(slTaskDotDirfMRI05) Response confirmation is set with keyboard key "3" \n')    
end
...
if (task.thistrial.thisseg == 5)
    ...
    if myscreen.responseType ~= 3
        ...
        %when response arrow is moved with keypresses "1"/"2"
        %register the reported estimate when confirmation key "3"
        %is pressed
...
%update stimulus displayed at each frame
...
if ~isfield(stimulus,'dots') || ~isfield(stimulus.dots,'rmax'), stimulus.dots.rmax = 9;end %15..(radius in angle)
