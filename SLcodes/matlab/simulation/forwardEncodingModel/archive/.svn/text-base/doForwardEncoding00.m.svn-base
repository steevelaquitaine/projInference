...
%purpose: run forward encoding modeling on matrix of BOLD (m voxels, n 
  ...
  %usage: [Ctrkn,channels,dirSpace,preferredDir,Weightmk]=doForwardEncoding(MTBOLDmn,6,1:1:360,'display=off');

  %description: all motion directions must be repeated to make sure the
  %weights we got from linear regression are correct!
  
function [Ctrkn,channels,dirSpace,preferredDir,Weightmk]=doForwardEncoding(MTBOLDmn,numchannel,motionDir,OptionDisp)
...
if rank(Ctrkn*Ctrkn')~=size(Ctrkn*Ctrkn')
    sprintf('The matrix of output channels is not full rank and thus not invertible. The weights solution are not unique')
else
    sprintf('Good...the matrix of output channels is invertible. Thus the weights are unique solutions')
    Weightmk=MTBOLDmn*(Ctrkn')*inv(Ctrkn*Ctrkn');
end
