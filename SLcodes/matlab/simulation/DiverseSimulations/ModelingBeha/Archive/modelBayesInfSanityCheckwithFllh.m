...
%Models
function [Sp,pred,fitP,R2,udata,sdata,dataOut,predOut,Fbp,F,stdPa,fitPt,logl]=modelBayesInfSanityCheckwithFllh(data,Fs,fig,FofLl2del,Ll2del)
pred=[];
fitP=[];
R2=[];
udata=[];
...
% Sp=GetPrior5(fitP.p,stdPa,[4.3 8.9 14.5 33],[.24 .12 .06]);
...
%You are free to set (or not,[]) your own free parameters (row vector).
fitP=[];%[6 12 24 80 40 20 10 0.01]';
[logl,fitP,pred,data,d,coh,pstd,stdPa]=MLft6(fitP);
fprintf(' \n %s \n','Now drawing model predictions....')
[meansD,meansP,stdsD,stdsP]=drawPred6(pred,data,d,coh,pstd);
Sp=GetPrior6(fitP.p,stdPa,[4.3 8.9 14.5 33],[.24 .12 .06]);
...
if isempty(fitP)==1
    ...
    %Initial parameters: prior's parameter are the experimental.
    sl1_0=250;%[250 350 450];%350;%k(1);
    sl2_0=24;%[14 24 34];%24;%k(2);
    sl3_0=7.5;%[1.5 7.5 17.5];%7.5;%k(3);
    sp1_0=20;%[1.3 4.3 14.3];%4.3;%k(4);
    sp2_0=40;%[1.9 8.9 18.9];%8.9;%k(5);
    sp3_0=60;%[4.5 14.5 24.5];%14.5;%k(6);
    sp4_0=300;%[23 33 43];%33;%k(7);
    ...
    tic;
    fitPbkp=nan(7,numel(sl1_0),numel(sl2_0),numel(sl3_0),numel(sp1_0),numel(sp2_0),numel(sp3_0),numel(sp4_0));
    logL_bkp=nan(numel(sl1_0),numel(sl2_0),numel(sl3_0),numel(sp1_0),numel(sp2_0),numel(sp3_0),numel(sp4_0));
    for i=1:numel(sl1_0)
        for j=1:numel(sl2_0)
            for k=1:numel(sl3_0)
                for l=1:numel(sp1_0)
                    for m=1:numel(sp2_0)
                        for n=1:numel(sp3_0)
                            for e=1:numel(sp4_0)
                                ...
                                [fitPtmp,fval,exitflag,output]=fminsearch(@(fitPtmp) makeSSE5(data,disp,coh,pstd,...
                                    fitPtmp),...
                                    [sl1_0(i);sl2_0(j);sl3_0(k);sp1_0(l);sp2_0(m);sp3_0(n);sp4_0(e)],...
                                    Options)
                                ...
                                fitPbkp(:,i,j,k,l,m,n,e)=fitPtmp;
                                logL_bkp(i,j,k,l,m,n,e)=logL;
                            end
                        end
                    end
                end
    ...
    [i,j,k,l,m,n,e]=ind2sub(size(logL_bkp),position);
    
    %best parameters
    fitP.p=fitPbkp(:,i,j,k,l,m,n,e);
    fitP.nm={'24','12','6','80','40','20','10'};
...
function [logl,fitP,pred,data,disp,coh,pstd,stdPa]=MLft6(fitP)
global databank
...
if isempty(fitP)==1
    ...
    %Initial parameters: prior's parameter are the experimental.
    sl1_0=250;%[250 350 450];%350;%k(1);
    sl2_0=34;%[14 24 34];%24;%k(2);
    sl3_0=7.5;%[1.5 7.5 17.5];%7.5;%k(3);
    sp1_0=8;%[1.3 4.3 14.3];%4.3;%k(4);
    sp2_0=14.5;%[1.9 8.9 18.9];%8.9;%k(5);
    sp3_0=28;%[4.5 14.5 24.5];%14.5;%k(6);
    sp4_0=50;%[23 33 43];%33;%k(7);
    pl0=1;
    ...
    %So k is between 0.001 and 700.
    tic;
    fitPbkp=nan(7,numel(sl1_0),numel(sl2_0),numel(sl3_0),numel(sp1_0),numel(sp2_0),numel(sp3_0),numel(sp4_0),numel(pl0));
    logL_bkp=nan(numel(sl1_0),numel(sl2_0),numel(sl3_0),numel(sp1_0),numel(sp2_0),numel(sp3_0),numel(sp4_0),numel(pl0));
    for i=1:numel(sl1_0)
        for j=1:numel(sl2_0)
            for k=1:numel(sl3_0)
                for l=1:numel(sp1_0)
                            ...
                            for e=1:numel(sp4_0)
                                for f=1:numel(pl0)
                                    ...
                                    [fitPtmp,logL,exitflag,output,~,~,Hessian]=fmincon( @(fitPtmp) makeSSE6(data,disp,coh,pstd,...
                                        fitPtmp),...
                                        [sl1_0(i);sl2_0(j);sl3_0(k);sp1_0(l);sp2_0(m);sp3_0(n);sp4_0(e);pl0(f)],...
                                        [],[],[],[],...
                                        [0 0 0 0 0 0 0 0],...
                                        [700 700 700 700 700 700 700 1],...
                                        [],...
                                        options)
                                    
                                    %search way 2 (Nelder-Mead)
                                    %[fitPtmp,logL,exitflag,output]=fminsearch(@(fitPtmp) makeSSE6(data,disp,coh,pstd,...
                                    %    fitPtmp),...
                                    %    [sl1_0(i);sl2_0(j);sl3_0(k);sp1_0(l);sp2_0(m);sp3_0(n);sp4_0(e);pl0(f)],...
                                    %    Options)
    ...
    fitP.nm={'24','12','6','80','40','20','10','pl'};
...
%scale the objective function to avoid scaling problems
%logL=logL*10^-10;
...
fprintf('\n %4.2f %4.2g %4.2g %4.2g %4.2g %4.2g %4.2g %4.2g \n',logL,kl1,kl2,...
    kl3,kp1,kp2,kp3,kp4)
...
%The probability of observing data in this model is the weighted sum of the 
%probabilty of observing data given a model with a strong likelihood and 
%the probability of observing data given a model with flat likelihood 
%(or logic).
%In this model "pl" is the weight and is constant across all conditions 
%and directions.
logLs=makelogl_K(data,displ,coh,Pstd,fitP);
fitPw=fitP;
fitPw(1:3)=0; 
logLw=makelogl_K(data,displ,coh,Pstd,fitPw);
pl=fitP(8);
logL=pl*logLs+(1-pl)*logLw;
...
fprintf('\n %4.2f %4.2g %4.2g %4.2g %4.2g %4.2g %4.2g %4.2g %4.2g \n',logL,kl1,kl2,...
    kl3,kp1,kp2,kp3,kp4,pl)
pred=makePre6(displ,coh,Pstd,[kl1 kl2 kl3 kp1 kp2 kp3 kp4 pl]);
drawPred5(pred,data,displ,coh,Pstd);
drawnow
...
%Create a measurement density at each trial which mean is the
%displayed direction and width is km=kl, the fit parameter calculated for
%this trial in "pl" of the trials (or 0 (1-pl) of the trials).

%Sample the measurement density and get a measurement mi for this
%trial. For trial-average prediction mi="displayed direction";

%mi=ul, likelihood's mean in this trial. Input ul, up(known), kl
%and kp (best fit parameters) in the equation that calculates the mean
%of the posterior (Murray and Morgenstern) for this trial, that is the
%trial predicted estimate.

%measurement densities (non flat and flat trials) and priors' strengths
up=225;
kml(coh==0.24)=k(1);
kml(coh==0.12)=k(2);
kml(coh==0.06)=k(3);
...
%%trial-average predictions are the weighted sum of the predictions for
%%flat and non flat llh trials.
mi=d;
prednoflat=ra2d(de2r(mi,1)+atan2(sin(de2r(up,1)-de2r(mi,1)),(kml./kp)'+cos(de2r(up,1)-de2r(mi,1))))';
predflat=ra2d(de2r(mi,1)+atan2(sin(de2r(up,1)-de2r(mi,1)),(kml0./kp)'+cos(de2r(up,1)-de2r(mi,1))))';
pred=pl*prednoflat+(1-pl)*predflat;
...
drawPublishAxis
function Sp=GetPrior6(freeP,stdPa,factor,factor2)
...
%in degrees
ul=ulall(I);
function [logL,fitP]=makelogl_K(data,displ,coh,Pstd,fitP)
