...
%Models
function [Sp,pred,fitP,R2,udata,sdata,dataOut,predOut,Fbp,F,stdPa,fitPt,logl]=modelBayesInf4(data,Fs,fig,FofLl2del,Ll2del)
pred=[];
fitP=[];
R2=[];
udata=[];
...
%.........................................................................
%model 5 (~30 min)
%Description: "Full Bayesian Model"
...
[logl,fitP,pred,data,d,coh,pstd]=MLft5(fitP);
fprintf(' \n %s \n','Now drawing model predictions....')
drawPred5(pred,data,d,coh,pstd);
...
function [logl,fitP,pred,data,d,coh,pstd]=MLft5(fitP)
global databank
...
%options
%iterations - 100 maxFunEvals lasts 40 min for 1 subject.
                ...
                for l=1:numel(sp1_0)
                    for m=1:numel(sp2_0)
                        for n=1:numel(sp3_0)
                            for e=1:numel(sp4_0)
                                %Fit
                                [fitPtmp,fval,exitflag,output,~,~,Hessian]=fmincon( @(fitPtmp) makeSSE5(data,disp,coh,pstd,...
                                    fitPtmp),...
                                ...
                                logL_bkp(i,j,k,l,m,n,e)=makeSSE5(data,disp,coh,pstd,fitPtmp);
                                t(1)=toc;
                            end
                            t(2)=toc;
                        end
...
if isempty(fitP.p)==1
    ...
    %iteration=0;
    tic
    fitPbkp=nan(numel(sl1_0),numel(sl2_0),numel(sl3_0),numel(sp1_0),numel(sp2_0),numel(sp3_0),numel(sp4_0),numel(sM_0));
    for i=1:numel(sl1_0)
        for j=1:numel(sl2_0)
            for k=1:numel(sl3_0)
                for l=1:numel(sp1_0)
...
figs={fig1,fig2};
function [means,stds]=drawPred5(pred,data,d,coh,pstd)
...
for j=1:F.f2.n
    ...
    for k=1:F.f1.n
        for i=1:F.f3.n
            %data
            D{k,i,j}=statcircular(data(F.inter.pos{k,i,j},:));
            meansD(k,i,j)=stat{k,i,j}.deg.mean;
        
            %pred
            P{k,i,j}=statcircular(pred(F.inter.pos{k,i,j},:));
            meansP(k,i,j)=P{k,i,j}.deg.mean;
            
            %draw
            hold all
            %data
            scatter(F.f1.L(k),meansD(k,i,j)',...
                'MarkerEdgeColor','w',...
                'MarkerFaceColor',F.f2.color{i},...
                'displayname',strcat(F.f2.nm,':',num2str(F.f2.L(j))))
            %pred
            scatter(F.f1.L(k),meansP(k,i,j)','.',...
                'MarkerEdgeColor','w',...
                'MarkerFaceColor','k',...
                'displayname',strcat(F.f2.nm,':',num2str(F.f2.L(j))))
...
%We use log likelihood because likelihood is so small than matlab cannot
%encode the likelihood properly.
logL=-sum(log(ML.p));
...
pred.mean=uPo;
pred.std=sEs;
function [pred,F]=makePre5(F,BestfitP)
%Create a measurement distribution at each trial which mean is the
%displayed direction and width is km=kl, the fit parameter calculated for
%this trial.

%Sample the measurement distribution and get a measurement mi for this
%trial.

%mi=ul, the mean of the likelihood in this trial. Input ul, up(known), kl
...
%For each condition, sample as many time as the condition occurred. Get the
%mean and std of the predicted estimate density for each condition and pass
%them to DrawPred function....
...
Sp.exp.raw=factor;
Sp.es.raw=freeP(4:7);
if isempty(stdPa)==0;Sp.std=stdPa(4:7);else Sp.std=[];end
...
Sl.es.raw=freeP(1:3);
if isempty(stdPa)==0;Sl.std=stdPa(1:3);else Sl.std=zeros(numel(Sl.es.raw),1);end
...
set(gca,'xtick',0.06:0.06:.3,'xticklabel',0.06:0.06:.3,'fontsize',20)
drawPublishAxis
