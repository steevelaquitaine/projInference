...
%usage: pred=simChangLLH(data,d,coh,pstd,k,'TrueMean',10);
       %pred=simChangLLH(data,d,coh,pstd,k,'TheoreticalMean',10);
...
%repeat simulations numsim times. There is no 'pstd' in cardinal simulation
data=repmat(data,numsim,1);
d=repmat(d,numsim,1);
coh=repmat(coh,numsim,1);
pstd=zeros(numel(data),1);
...
%PpriorSwitch: probability of response bias.

%useful variables
ntrial=numel(d);
xe=1:1:360;

%strength of the measurement distributions, cardinal prior & motor noise
kml(coh==0.24)=k(1);
kml(coh==0.12)=k(2);
kml(coh==0.06)=k(3);
kp(1:ntrial)  =k(4);
...
parfor i=1:ntrial
    ...
    vm=vmPdfs(xe,[90 180 270 360],kpi,'norm');
    prior=0.25.*sum(vm,2);
    ...
    %(number of floating points). Prandomoblem is that some resolutions are
    ...
    %oblique.
    [~,upo1(i)]=max(posterior);
    if mi==45||mi==135||mi==225||mi==315
        if upo1(i)<mi
            upo1Atobliques=[upo1(i) mi+(mi-upo1(i))];
        else
            upo1Atobliques=[mi+(mi-upo1(i)) upo1(i)];
        end
...
PpriorSwitchi=rand(ntrial,1)<PpriorSwitch;

%random estimates trials
Prandomi=zeros(ntrial,1);
ntrial_BIweak=sum(PpriorSwitchi);
...
parfor trial=1:ntrial
    llh=vmPdfs(xe,d(trial),kml(trial),'norm');
    vm=vmPdfs(xe,[90 180 270 360],kpi,'norm');
    prior=0.25.*(sum(vm,2));
    posterior=llh.*prior/sum(llh.*prior);
    ...
    %the oblique mi.
    [~,predmean(trial)]=max(posterior);
    if d(trial)==45||d(trial)==135||d(trial)==225||d(trial)==315
        predmean(trial)=d(trial);
    end
end
pred.mean=predmean;
...
%switching to the prior "PpriorSwitch".
Kreal=k;
Kreal(5)=mixtP.PrandomReal;
Kreal(7)=mixtP.PpriorSwitchReal;
