...
%Girshick estimator with cardinal priors for given prior & likelihood 
%strengths.

%Usage
    %k.m=[10 10]; 
    %k.p=[0 42]; 
    %[mPdfs,l,prior,po,di,m,uniqMAP,whichplot]=GirshickEs4predWithBimodalPrior(k,[0.9 0.65 0.5;
    %0.8 0 0];)
...
function [mPdfs,l,prior,po,di,m,uniqMAP,whichplot]=GirshickEs4predWithCardinalPrior(k,colorM)
...
%prior (over motion directions (row) is same for each mi(col))
prior=vmPdfs(di,[90 180 270 360],k.p,'norm');
prior=0.25.*(prior(:,1)+prior(:,2)+prior(:,3)+prior(:,4));
prior=prior(:,ones(numel(m),1));
...
%of ul/mi given the motion direction for each ul and associaed upo. 
%ul/mi
