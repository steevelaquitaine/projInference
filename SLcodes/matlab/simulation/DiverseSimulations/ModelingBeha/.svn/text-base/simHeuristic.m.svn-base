...
function [pred,Kinput,Kreal,mixProba]=simHeuristic(data,d,coh,pstd,k,meanType,numsim)
...
parfor i=1:ntrial
    ...
    %1. perceive likelihood mean ('seeing')
    %percept=measurement sample
    mi(i)=randsample(xe,1,true,p_meas(:,i));
    upo1(i)=mi(i);
    
    %2. perceive prior mean ('not seeing')
    upo2(i)=225;
