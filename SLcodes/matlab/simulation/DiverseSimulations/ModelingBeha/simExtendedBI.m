...
%Author: Steeve Laquitaine
  %date: 131024
  %goal: Simulate extended Bayesian inference with cardinal bias (Next is
  %with "oblique effect" (inhomogeneous likelihood, Girshick etal))
...
function [pred,Kinput,Kreal]=simExtendedBI(data,d,coh,pstd,k,meanType,numsim)
...
%llhs (p(directions|mi))
for i=1:numel(xe)
    measurementdensity24(:,i)=vmPdfs(xe,xe(i),k(1),'norm');
    measurementdensity12(:,i)=vmPdfs(xe,xe(i),k(2),'norm');
    measurementdensity06(:,i)=vmPdfs(xe,xe(i),k(3),'norm');
end
llh24=mea

%Calculate the percepts produced by the 3 processes.
parfor i=1:ntrial
    %mixture of priors ("learnt" + "cardinals")
    %learnt
    learntPrior(:,i)=vmPdfs(xe,up,kp(i),'norm');
    cardinalPrior(:,i)=...
        0.25*vmPdfs(xe,360,kcardinal,'norm')+...
        0.25*vmPdfs(xe,90,kcardinal,'norm')+...
        0.25*vmPdfs(xe,180,kcardinal,'norm')+...
        0.25*vmPdfs(xe,270,kcardinal,'norm');
    mixPrior(:,i)=0.5*learntPrior(:,i)+0.5*cardinalPrior(:,i);
...
for j=1:F.f2.n
    ...
    for i=1:F.f3.n
        ...
        plot(F.f1.L,225*ones(31,1),'b.','Markersize',4,'lineSmoothing','on')
        xlim([30 365])
        ylim([30 365])
        ...
        %std
        subplot(hs(2))
        plot(225*ones(31,1),F.f1.L,'b.','Markersize',5,'lineSmoothing','on')
        xlim([30 365])
        ylim([0 150])
        set(gca,'xtick',[30 225 365],'xticklabel',{'30','225','365'},'fontsize',ft)
