...
C = (nansum(X(:,1)))^2/nansum(isnan(X(:,1))==0); %correction term
SST = nansum(X(:,1).^2)-C; %total sum of squares
dfT = nansum(isnan(X(:,1))==0)-1; %total degrees of freedom

SSA = nansum(A)-C; %IV sum of squares
v1 = k-1; %IV degrees of freedom
SSS = nansum(S)-C; %within-subjects sum of squares
