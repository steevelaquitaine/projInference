...
% SLBreakBar(sampleTimes,RPM,1500,5000,'Line','facecolor','r');
 
% SLBreakBar([1 2 3],[2 1000 2000],360,600,'Line','facecolor','r');


%inspired from BreakPlot matlab file exchange


function h = SLBreakBar(x, y, y_break_start, y_break_end, break_type,...
    varargin)
...
%case more that one value is plotted
if i~=1
    y=(ytick(i)-ytick(i-1))./2+ytick(i-1);
else
    y=(ytick(i)-ytick(i))./2+ytick(i);
end
