...
%slMakeLocStimBlurEdg.m
%
%
% author: steeve laquitaine
%   date: 151112
%purpose: draw a blurred edge at different locations
%
%  usage:
%
%       slMakeLocStimBlurEdg('displayName=Test','sigmaw=15')
%
%
%
%Description:
%
%   Location contrast stimulus. Stimulus is a blurred
%   oriented edge (hallow of light) on a gray background


function myscreen = slMakeLocStimBlurEdg(varargin)
...
params.task.parameter.loc.con   = repmat([0.05 0.01 0.002],36,1);%repmat([0.0009 0.0018 0.0036],36,1);
...
for i = 1 : numtrials
    ...
    bkg2 = stimulus.colors.grayColor*ones(ImgH,ImgW); %gray
    ...
    %angMask = slGraphWegdes(params.task.parameter.loc.series(i),ImgW,ImgH,200);   
    loc = params.task.parameter.loc.series(i);    
    angMask = slGraphBlurEdge(ImgH,ImgW,loc,200,sigmaw);
...
%mask with 1% contrast at max, 0% at mean
LumBg = gammaTableNew(stimulus.colors.midStimIx);
angMask2 = angMask + LumBg - min(angMask(:));
cUnit = (angMask2 - min(angMask2(:)))/(max(angMask2(:))-min(angMask2(:)));

%Michelson contrast: (LumMax-LumBg)/(LumMax+LumBg)
%this change from simply substracting min and max lum
%because in this stimulus display Imax + Imin don't sum to 1.
LumMax = (curContrast*LumBg + LumBg)/(1-curContrast);
difLum = LumMax - LumBg;
angMask2 = LumBg + cUnit*difLum;
...
angMask2 = round(angMask2*1e4)/1e4; %speed up
uM = unique(angMask2(:));
angMaskIx255 = nan(size(angMask2));
for i = 1: length(uM)
    [~,ix] = min(abs(bsxfun(@minus,gammaPoss(:,1),uM(i))));   
    angMaskIx255(angMask2==uM(i)) = ix;
end

%express ix between 5 and 255
%for mglCreateTexture
angMaskIx255 = angMaskIx255 + stimulus.colors.nresvd;

%actual max contrast
actualCon = max(angMask2(:)) - min(angMask2(:));
