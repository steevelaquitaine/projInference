...
%slMakeLocStimWdgAreaBgFlash2.m
%
%
...
%
%       hi-pass
%       slMakeLocStimWdgAreaBgFlash2('displayName=test','sigmaw=30'); %better ! (more gradient ?)
%
%       lo-pass
%       slMakeLocStimWdgAreaBgFlash2('displayName=test','sigmaw=2')
%
%Description:
%
%       My favorite so far:
...
%       flashing cloudy bg
%
%In progress... fixed contrast bg with controlled contrast location


function myscreen = slMakeLocStimWdgAreaBgFlash2(varargin)
...
%-----------------------------init Task----------------------------
task{2}{1}.segmin = [1 .1 4];
task{2}{1}.segmax = [1 .1 4];
...
params.task.parameter.loc.con   = repmat([1 .05 .02],36,1);%repmat([0.0009 0.0018 0.0036],36,1);
...
stimulus.FlickerFreq   = 4; %8 %flickering frequency in hertz
...
numtrials = 10;
%--------------
%load precomputed stimulus
% mglClose; keyboard
if slIsInput(varargin,'loadPrecompStim')
    load('stim_slMakeLocStimWdgAreaBgFlash')
    slPrintfStr('slMakeLocStimWdgAreaGrBg','Loading precomp stimuli')
    %create stimuli
    for i = 1 : numtrials
        %input are indices between 5 and 255
        task{2}{1}.randVars.texFls{i} = mglCreateTexture(bkgFl{i});
        task{2}{1}.randVars.tex{i} = mglCreateTexture(bkg2{i});
    end
else
    %or compute stim
    ...
    for i = 1 : numtrials
        ...
        bkg2{i} = bkgfsc*(stimulus.colors.nPossCol-1)+stimulus.colors.nresvd+1;
        %flashed bg
        bkgFl{i} = bkg2{i};
        ...
        %enhance contrast at wedge location
        %[actualCon(i),angMaskIx255] = getNormIdxContrast(task{2}{1}.randVars.myRandomCon(i),gammaTableNew,bkg2{i},angMask);
        %control bg contrast at 1% max        
        BgMask = ones(size(bkgFl{i},1),size(bkgFl{i},2));
        [actualBgCon(i),bgIx255] = getNormIdxContrastBg(0.01,gammaTableNew,bkgFl{i},BgMask);
        bkgFl{i} = bgIx255;
        %control stim contrast
        [actualCon(i),angMaskIx255] = getNormIdxContrast(task{2}{1}.randVars.myRandomCon(i),gammaTableNew,bkgFl{i},angMask);        
        bkgFl{i}(angMask==1) = angMaskIx255; 
%         mglClose; keyboard
        %input are indices between 5 and 255
        task{2}{1}.randVars.texFls{i} = mglCreateTexture(bkgFl{i});
        task{2}{1}.randVars.tex{i} = mglCreateTexture(bkg2{i});
...
save('stim_slMakeLocStimWdgAreaBgFlash','bkg2','bkgFl')
...
setGammaTableForMaxContrast(stimulus.maxCon);
Graytex = mglCreateTexture(stimulus.colors.midStimIx);

%update frame count each frame
stimulus.countFrame = stimulus.countFrame + 1;
%update bkg only when frame #satisfying flickering
%freq. is reached otherwise gray
if SLisMultiple(stimulus.countFrame,stimulus.nFrameStill);
    stimulus.currentFlicker = stimulus.currentFlicker + 1;
end
if iseven(stimulus.currentFlicker)
    mglBltTexture(task.thistrial.texFls,[0 0]);
else
    mglBltTexture(Graytex,[0 0]);
end

%location stim
%coor centered at screen center
if (task.thistrial.thisseg == 2)
...
function [actualCon,lumIx] = getNormIdxContrastBg(curContrast,gammaTableNew,backLum,angMask)

global stimulus

%location mask luminance in
%new gamma table
%1 to 255
angMaskIx255 = round(backLum(angMask==1));
%mid lum
midLum = gammaTableNew(stimulus.colors.midStimIx,1);
LumMax = midLum + curContrast/2;
LumMin = midLum - curContrast/2;
%scale all luminances between max and min 
%required for curContrast
%The current contrast is the current max contrast
%observed between the brightest and darkest pixels
LumTables = LumMin:(LumMax - LumMin)/255:LumMax;
lum = LumTables(angMaskIx255);
%get unique luminance to speed up the loop
%beow
lumUn = unique(lum);
%possible luminances in the gamma table
gammaPoss = gammaTableNew(stimulus.colors.minStimIx+1:end,:);
%indices of nearest pixel luminances in
%the gammaTable required for current contrast
for i = 1 : length(lumUn)    
    [~,ix(i,:)] = min(abs(bsxfun(@minus,gammaPoss,[lumUn(i) lumUn(i) lumUn(i)])));
    lumIx(lum==lumUn(i)) = ix(i,1);    
end
%make ix between 5 and 255
%for mglCreateTexture
lumIx = lumIx + stimulus.colors.nresvd;
%back to image sizes
lumIx = reshape(lumIx,size(backLum,1),size(backLum,2));
%bg actual max contrast
actMaxLum = gammaTableNew(max(lumIx(:)),1);
actMinLum = gammaTableNew(min(lumIx(:)),1);
actMeanLum = mean(gammaTableNew(lumIx(:),1));
actualCon = (actMaxLum - actMinLum)/actMeanLum;
...
% %1 to 255
% angMaskIx255 = round(backLum(angMask==1));
% %scale between min and max Ix (5 and 255)
% li = angMaskIx255 > mean(angMaskIx255);
% da = angMaskIx255 < mean(angMaskIx255);
% aM = 0.5*ones(length(angMaskIx255),1);
% aM(li) = 0.5+0.5*(angMaskIx255(li) - mean(angMaskIx255))./(max(angMaskIx255(li)) - mean(angMaskIx255));
% aM(da) = 0.5-0.5*(angMaskIx255(da) - mean(angMaskIx255))./(min(angMaskIx255(da)) - mean(angMaskIx255));
% scaledIx = round(aM*(stimulus.colors.nPossCol-1)+stimulus.colors.nresvd+1);
% %mid lum
% midLum = gammaTableNew(stimulus.colors.midStimIx,1);
% LumMax = midLum + curContrast/2;
% LumMin = midLum - curContrast/2;
% if LumMax>1;LumMax = 1; end
% if LumMin<0;LumMin = 0; end
% %scale all luminances between max and min 
% %required for curContrast
% %The current contrast is the current max contrast
% %observed between the brightest and darkest pixels
% LumTables = LumMin:(LumMax - LumMin)/255:LumMax;
% lum = LumTables(scaledIx);
% %get unique luminance to speed up the loop
% %beow
% lumUn = unique(lum);
% %possible luminances in the gamma table
...
% %for mglCreateTexture
% lumIx = lumIx + stimulus.colors.nresvd;
% %bg actual max contrast (Michelson)
% actMaxLum = gammaTableNew(max(lumIx(:)),1);
% actMinLum = gammaTableNew(min(lumIx(:)),1);
% actMeanLum = mean(gammaTableNew(lumIx(:),1));
% actualCon = (actMaxLum - actMinLum)/(actMaxLum+actMinLum);
% 
