...
%slMakeLocStimInCloud.m
%
%
% author: steeve laquitaine
%   date: 151112
%  usage:
%purpose: display uniformly bright radial lines location stimulus
%         on cloudy background
%
%       slMakeLocStimInCloud('displayName=Test','sigmaw=10')
%
% Description:
%
%           uniformly bright radial lines displayed at different 
%           locations superimposed on cloudy contrasted background


function myscreen = slMakeLocStimInCloud(varargin)
...
%# of possible stim. luminances (e.g., 255 - nres)
maxIx = 255;
...
for i = 1 : length(task{2}{1}.randVars.myRandomloc)
    [actualCon(i),LumMaxIx(i),LumMinIx(i),ixMax(i),ixMin(i)] = getNormIdxContrast(task{2}{1}.randVars.myRandomCon(i),gammaTableNew);
end
task{2}{1}.randVars.actualCon = actualCon;
task{2}{1}.randVars.LumMax = LumMaxIx; %0:1 indx max
task{2}{1}.randVars.LumMin = LumMinIx;
task{2}{1}.randVars.ixMax = ixMax; %1:255 indx max
task{2}{1}.randVars.ixMin = ixMin;
...
for i = 1: numtrials
    ...
    bkg2 = bkgfsc*stimulus.colors.nPossCol+stimulus.colors.nresvd+1;

    %create stimulus here to add to noisy background texture
    angMask = slGraphWdgLines(params.task.parameter.loc.series(i),ImgW,ImgH,200);
    bkg2(angMask==1) = ixMax(i)-1;
    
    %input are indices from 1:255
...
function [actualCon,LumMaxIx,LumMinIx,ixMax,ixMin] = getNormIdxContrast(curContrast,gammaTableNew)
...
%indices in the table
[~,ixMax] = min(abs(bsxfun(@minus,gammaPoss,[LumMax LumMax LumMax])));
[~,ixMin] = min(abs(bsxfun(@minus,gammaPoss,[LumMin LumMin LumMin])));
ixMax = ixMax(1);
ixMin = ixMin(1);
...
%actual contrast
actualCon = gammaTableNew(ixMax) - gammaTableNew(ixMin);
...
%luminances
normIx = [0 [1:255]/255];
LumMaxIx = normIx(ixMax);
LumMinIx = normIx(ixMin);
