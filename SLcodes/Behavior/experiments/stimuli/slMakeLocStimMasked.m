...
%       
%       lo-f bg
%       slMakeLocStimMasked('displayName=Test','sigmaw=15')
%
%       hi-f bg
%       slMakeLocStimMasked('displayName=Test','sigmaw=2')
%
%
%
%Description:
%
%       brief contrasted black and white flickering wedge 
%       displayed at different locations on gray Bg 
%       followed briefly by low pass-filtered bg 
%       (black and white mask)


function myscreen = slMakeLocStimMasked(varargin)
...
task{2}{1}.segmin = [1 .3 .017];
task{2}{1}.segmax = [1 .3 .017];
...
params.task.parameter.loc.con   = repmat([0.05 0.03 0.02],36,1);%repmat([0.0009 0.0018 0.0036],36,1);
...
stimulus.FlickerFreq   = 60;%8 %flickering frequency in hertz
...
for i = 1 : numtrials
    ...
    %reserved in the gamma table
    bkg2 = nan(ImgH,ImgW);
    bkg2(bkgfsc>0.5) = stimulus.colors.resvdIx(1)-1; %black    
    bkg2(bkgfsc<0.5) = stimulus.colors.resvdIx(2)-1; %white
