...
%      usage: taskDotDir
%         by: justin gardner modified by Steeve Laquitaine 12/10/07
%       date: 09/07/06
%  copyright: (c) 2006 Justin Gardner (GPL see mgl/COPYING)
%    purpose: random dot motion direction discrimination task
...
task{2}{1}.randVars.myStd   = params.task.parameter.dir.std;
...
%     
    ...
    % Interface PowerMate
    [mouseinfo] = PowerMate2ap(mouseinfo,stimulus);
    % Collect response
...
%-------------------------------------------------------------------
function [mouseinfo]=PowerMate2ap(mouseinfo,stimulus)
global Inicoord;
% Set speed, initial position and size of arrow.
wheelspeed = 0.008;%0.02; %0.008;
initAngle  = (225+270)/2*pi/180;
r = stimulus.dots.rmax;
% Calculate arrow's coordinates on aperture
mouseinfo.x.angle.onap =  r * cos(initAngle + wheelspeed*(mouseinfo.x.pix - Inicoord.x.pix.onap)); % (-) --> goes left when turn left, vvs.
mouseinfo.y.angle.onap =  r * sin(initAngle + wheelspeed*(mouseinfo.x.pix - Inicoord.x.pix.onap));
