...
%% Test bimodal priors
% [130409]
% test first with an intermediate prior(good raneg of displayed directions
% for comparison) at lowest coherence, when the prior is strong.
% [task] = initRun('steeve_exp02_Bimodal_metho_Pmodes205_245_Pstd012_mean225_coh6_12_24_t107_75_33_dir36_130412', 225, [205 245], 12,[.06 .12 .24],[107 75 33],5:10:355)

% [task] = initRun('steeve_exp02_Bimodal_metho_Pmodes145_305_Pstd012_mean225_coh6_12_24_t107_75_33_dir36_130412', 225, [145 305], 12,[.06 .12 .24],[107 75 33],5:10:355)

% [task] = initRun('steeve_exp02_Bimodal_metho_Pmodes185_265_Pstd012_mean225_coh6_12_24_t107_75_33_dir36_130412', 225, [185 265], 12,[.06 .12 .24],[107 75 33],5:10:355)

% [task] = initRun('steeve_exp02_Bimodal_metho_Pmodes165_285_Pstd012_mean225_coh6_12_24_t107_75_33_dir36_130412', 225, [165 285], 12,[.06 .12 .24],[107 75 33],5:10:355)
...
%%% Design a run
%-------------------------------------------------------------------------
function [task] = initRun(Prname, Prmean,Prmodes,Prstd, coh, Prnumtrials_perCoh, PrSp)
% Initialize figure
fig.hdle = figure(1);
set(gcf,'color','w')
fig.name = Prname; 
...
for i = 1:numel(Prnumtrials_perCoh)
    ...
    [Priortmp{i}] = initPriors(Prname, Prmean, Prmodes, Prstd, Prnumtrials_perCoh(i), PrSp);
    % Initialize task
    task = Priortmp{1};
...
%%% Design Prior
%-------------------------------------------------------------------------
function [task] = initPriors(Prname, Prmean, Prmodes, Prstd, Prnumtrials, PrSp)
% fig.name=Prname; 
%mean of the distribution (stay the same).
% Set the mean of the prior
task.parameter.dir.mean   = Prmean;   
% Set the modes of the prior
...
% bimodal gaussian (MoG)
function [functionf] = MoG( x, modes )
%130408 - Steeve
%MoG is a weighted sum of component gaussians: p(x) = Sum_k_to_K( w(k) * N(x\mu(k), sigma(k)) )
% x = 5:10:355
% Fixed parameters of the bimodal distribution
mu(1) = modes(1);
mu(2) = modes(2);
s(1) = 12;
s(2) = 12;
...
% Create distribution
f = w   * (1/(s(1)*sqrt(2*pi))) * exp( -.5 * ((x - mu(1))/s(1)) .^ 2 ) + ...
   (1-w)* (1/(s(2)*sqrt(2*pi))) * exp( -.5 * ((x - mu(2))/s(2)) .^ 2 );
