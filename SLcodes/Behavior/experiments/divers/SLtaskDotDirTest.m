...
%   purpose: ran1111dom dot motion direction estimation task

function myscreen = SLtaskDotDirTest(params,varargin)

%Test mode
if isempty(params)
    
    fprintf('(SLtaskDotDirTest) I am running the Random dot task in test mode \n')
        
    %set default parameters
    %stimulus (everything randomized)
    task{2}{1}.randVars.myRandomDir = randsample(1:1:360,360,true,vmPdfs(1:1:360,225,7,'norm'));
    task{2}{1}.randVars.myRandomCoh = randsample([.06 0.12 .24],360,true,[0.33 0.33 0.33]);
    task{2}{1}.randVars.myMean  = 225;
    task{2}{1}.randVars.myModes = 225;
    task{2}{1}.randVars.myStrength = 7;
    task{2}{1}.randVars.myDirections   = unique(task{2}{1}.randVars.myRandomDir);
    ...
    task{2}{1}.randVars.myModes = params.task.parameter.dir.modes;
    task{2}{1}.randVars.myStrength   = params.task.parameter.dir.strength;
    task{2}{1}.randVars.myDirections   = params.task.parameter.dir.sample;
end

%check options11
if ~isempty(varargin)
    pos = find(strcmp(varargin,'displayName'));
    displayName = varargin{pos+1};
else
    fprintf('(SLtaskDotDirTest) Please set at least the displayName')
    keyboard
end
...
if (task.thistrial.thisseg == 4)
    ...
    %set screen to 1 if test screen (0)
    if myscreen.screenNumber==0 ||myscreen.screenNumber==1
        myscreen.screenGetMouse = 1;
    else
        myscreen.screenGetMouse = 2;
    end
    
    %Get mouse position
    mi = mglGetMouse(myscreen.screenGetMouse); %get pixel positions.
...
if (task.thistrial.thisseg == 3)
    ...
    %Position mouse on aperture
    mi = mglGetMouse(myscreen.screenGetMouse); %(pixels)
    mouseinfo.x.pix = mi.x;
    mouseinfo.y.pix = mi.y;
    mouseinfo = pos2ap(myscreen,fixStimulus,mouseinfo,stimulus);
...
if (task.thistrial.thisseg == 4)
    ...
    mi = mglGetMouse(myscreen.screenGetMouse); 
    mouseinfo.x.pix=mi.x;
    mouseinfo.y.pix=mi.y;
    mouseinfo=pos2ap(myscreen,fixStimulus,mouseinfo,stimulus);
...
fprintf([num2str(stimulus.dots.n) ' dots'])
