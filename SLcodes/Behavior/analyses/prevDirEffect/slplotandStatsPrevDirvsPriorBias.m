...
%slplotandStatsPrevDirvsPriorBias.m
%
% author: steeve laquitaine
%purpose: plot the average bias toward the prior versus the previous motion
%         direction relative to the current direction for each subject
%         Do that stats
%         Test the null hypothesis that the average overall distance between the 
%         estimate and the prior over all the trials in the task is > 0 and
%         that its lower 95% confidence is larger than 0 (bias toward prior)
%         SigBias2priorAll = 1 means significant bias toward the prior
%
%         Test the null hypothesis that the average overall distance between the 
%         estimate and the previously displayed direction over all the trials 
%         in the task is < 0 and that its upper 95% confidence is lower 
%         than 0 (bias toward previous displayed direction)
%         SigBias2prevdirAll = 1 means significant bias toward the previous
%         direction
%
%usage :
%
%   [SigBias2priorAll, SigBias2prevdir, marginAll,ciAll,mAll,sAll] = slplotandStatsPrevDirvsPriorBias({'sub01'},'~/data/dataPsychophy/proj01_priorStrength/')

function [SigBias2priorAll, SigBias2prevdirAll, marginAll,ciAll,mAll,sAll]  = slplotandStatsPrevDirvsPriorBias(subject,datapath)
...
%< 0) ?
SigBias2prevdirAll = mAll<0 & ciAll(2)<0;
