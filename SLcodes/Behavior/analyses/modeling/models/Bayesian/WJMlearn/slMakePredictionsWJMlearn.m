...
%  usage: 
%
%      [Pdata,pPred,Bins,o] = slMakePredictionsWJMlearn({'sub01'},[3 0.7 2.7 8 33 1e-2 32],'experiment','vonMisesPrior')
%
%
%
%
...
%Predictions:
%
% - coherence is also a parameter of the model. Thus for the same sensory
%   strength kappa different coherences produce different estimates density.
%   This is different from Bayesian model that models Likelihood with a
%   probability density that peaks at the coherent motion with circular noise
%   kappa.

function [Pdata,pPred,Bins,o] = slMakePredictionsWJMlearn(subjects,simp,varargin)
...
%get data
%--------
fprintf('%s \n','(slMakePredictionsWJMlearn) Gathering data ...')
fprintf('%s \n','(slMakePredictionsWJMlearn) Please set dataPath ...')
...
for i = 1 : length(coh)
    ...
    if mod(round(rotdeg{i}),svecStep)
        fprintf('(slMakePredictionsWJMlearn) Something"s wrong. Please change Ns. Ns=72 usually works fine.')
    end
...
%--------------------
tic
[logl,~,PestGivModel] = SLgetLogL_WJMlearnrotatedDir(data,s,cu,kappa,kappa_s,prand,km,simp,o.posC,o.uniqCond);
o.duration = toc;
o.logl     = logl;
...
%backup
%-------
save('slMakePredictionsWJMlearn')
fprintf('(slMakePredictionsWJMlearn) Saving data in "dataslMakePredictionsWJMlearn". \n')
