...
%SLgetLoglBayesianModelUnifPrior.m
%
% author: steeve laquitaine
%   date: 160606
%purpose: get the log likelihood that a maximum likelihood model (Bayes 
%         with uniform priors) produce observed data
%         
%
%
%
%usage:
%
%       [negLogl,fitP,Logl_pertrial,PestimateGivenModel,MAP] = SLgetLoglBayesianModelUnifPrior(data,displ,coh,pstd,fitP,'bimodalPrior',priorModes,TheModel,'bimodalPrior','MAPReadout')
%
%
%varargin options
%
...
%logl
function [negLogl,fitP,Logl_pertrial,PestimateGivenModel,MAP,AIC,PdataGivenModel] = SLgetLoglBayesianModelUnifPrior(data,...
    displ,...
    coh,...
    pstd,...
    fitP,...
...
%----------------------------------------
%Model structure is the same but the estimate for each displayed motion
...
%adjust old code to implement uniform prior
fitP = [fitP(1:3) 0 0 0 0 fitP(4:end)];
...
if sum(strcmp(priorShape,'vonMisesPrior'))
    ...
    elseif sum(strcmp(varargin{:},'FatTailPriorAndLLH'))==1 && sum(strcmp(varargin{:},'FitEachKvmAndTails'))==0
    ...
    elseif sum(strcmp(varargin{:},'FatTailPriorAndLLH'))==1 && sum(strcmp(varargin{:},'FitEachKvmAndTails'))==1
    ...
    elseif sum(strcmp(varargin{:},'FatTailPrior'))==0 && sum(strcmp(varargin{:},'FatTailPriorAndLLH'))==0
...
for i = 1 : numel(motdir)
    ...
    for j = 1 : numPriors
        ...
        %coh 24,12,6
        PupoGivenBI(:,thisd&coh24&Prior(:,j)) = likeMAP1{j}(:,motdir(i(ones(sum(thisd&coh24&Prior(:,j)),1))));        
        PupoGivenBI(:,thisd&coh12&Prior(:,j)) = likeMAP2{j}(:,motdir(i(ones(sum(thisd&coh12&Prior(:,j)),1))));
        PupoGivenBI(:,thisd&coh06&Prior(:,j)) = likeMAP3{j}(:,motdir(i(ones(sum(thisd&coh06&Prior(:,j)),1))));
    end
end
