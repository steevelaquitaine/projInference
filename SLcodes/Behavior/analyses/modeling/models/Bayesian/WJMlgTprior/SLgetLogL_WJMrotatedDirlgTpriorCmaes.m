%SLgetLogL_WJMrotatedDirlgTpriorCmaes.m
%
%
% author: steeve laquitaine
%   date: 150909
%purpose: calculate logl of motion direction estimation
%         data given WJM model with long-tailed priors and best predicted 
%         estimates densities for fit with Cmaes procedure
%           
%  usage:
%
%       [loglAll,fitP,Pest] = SLgetLogL_WJMrotatedDirlgTpriorCmaes(data,s,c,kappa,kappa_s,prand,km,fp,posC,Cond)
%
%  inputs:
%           data
%              s
...
%get logl of data
function [neglogl,fitP,Pest] = SLgetLogL_WJMrotatedDirlgTpriorCmaes(fp,data,...
    s,...
    c,...
    kappa,...
    kappa_s,...
...
if any(isnan(fp))
    neglogl = 1e09;
    fprintf('%s \n','(SLgetLogL_WJMrotatedDirlgTpriorCmaes) A param is NaN')
    dbstack
    keyboard
end
...
%without changing its shape. besseli(0,k,n1)) does same. The adjusted
