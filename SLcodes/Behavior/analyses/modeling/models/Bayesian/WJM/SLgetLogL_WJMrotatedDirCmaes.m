%SLgetLogL_WJMrotatedDirCmaes.m
%
%
...
%         for fit with cmaes procedure
%
%  usage:
%
%       [loglAll,fitP,Pest] = SLgetLogL_WJMrotatedDirCmaes(data,s,c,kappa,kappa_s,prand,km,fp,posC,Cond)
%
%
% outputs:
%
...
%get logl of data
function [neglogl,fitP,Pest] = SLgetLogL_WJMrotatedDirCmaes(fitP,data,...
    s,...
    c,...
    kappa,...
    kappa_s,...
...
if any(isnan(fitP))
    neglogl = 1e09;
    fprintf('%s \n','(SLgetLogL_WJMrotatedDir) A param is NaN')
    dbstack
    keyboard
end
...
x1_24     = slCirc_vmrnd2(su24(1),fitP(1),Ntrials,Ndots);
x1_12     = slCirc_vmrnd2(su12(1),fitP(1),Ntrials,Ndots);
x1_6      = slCirc_vmrnd2(su6(1) ,fitP(1),Ntrials,Ndots);
x1        = [{x1_24} {x1_12} {x1_6}];
...
%prior 80
if fitP(2) <= 1e300
    prior80   = bsxfun(@plus,M,2/pi/besseli(0,fitP(2),1)*exp(fitP(2) * cos(svec - pmean) - fitP(2))); %Mean of prior is 225 deg (3.93 rads),need high precision
else
    %delta density
...
%prior 40
if fitP(3) <= 1e300
    prior40   = bsxfun(@plus,M,2/pi/besseli(0,fitP(3),1)*exp(fitP(3) * cos(svec - pmean) - fitP(3)));
else
    %delta density
...
%prior 20
if fitP(4) <= 1e300
    prior20   = bsxfun(@plus,M,2/pi/besseli(0,fitP(4),1)*exp(fitP(4) * cos(svec - pmean) - fitP(4)));
else
    %delta density
...
%prior 10
if fitP(5) <= 1e300
    prior10   = bsxfun(@plus,M,2/pi/besseli(0,fitP(5),1)*exp(fitP(5) * cos(svec - pmean) - fitP(5)));
else
    %delta density
...
for i = 1 : length(coh)
    tail   = tailtmp(i);
    amp    = amp1tmp(i)/besseli(0,fitP(1),1);
    shift  = shifttmp{i};
    Pesttmp = getPestEachCoh(Ntrials, Ndots, Ns, svec, su{i}, Nsu{i} ,x1{i} ,rotrad{i}, coh(i),fitP(1),prior80,prior40,prior20,prior10,prand,motn);%Pest coh 24%
