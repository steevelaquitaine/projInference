%SLwjmPrefitInitpSub2to4onBowmore.m
%
...
%  usage:
%
%       o = SLwjmPrefitInitpSub2to4onBowmore({'sub02','sub03'},[2 2 2 2.3 2.3 2.3 2.3 1 3],'experiment','vonMisesPrior') 
%    
%
...
function o = SLwjmPrefitInitpSub2to4onBowmore(subjects,initp,varargin)
...
%---------------------
fprintf('%s \n','(SLwjmPrefitInitpSub2to4onBowmore) Gathering data ...')
fprintf('%s \n','(SLwjmPrefitInitpSub2to4onBowmore) Please set dataPath ...')
dataPath = uigetdir(cd,'Pick a project e.g., /dataPsychophy/Exp01...');

if ~isempty(dataPath)
    cd(dataPath)
    fprintf('(SLwjmPrefitInitpSub2to4onBowmore) Data path was set to : \n')
    fprintf(['(SLwjmPrefitInitpSub2to4onBowmore) "',dataPath,'" \n'])
else
    fprintf('(SLwjmPrefitInitpSub2to4onBowmore) Data path was not set...aborting \n')
    keyboard
end
...
parfor sub = 1 : length(subjects)
    ...
    fprintf('(SLwjmPrefitInitpSub2to7onSuze) Getting the subjects data...\n')
    ...
    fprintf('%s %i %s \n','(SLwjmPrefitInitpSub2to7onSuze) - subjects ',sub,' ...done')
...
for sub = 1 : length(subjects); 
    savedfNames{sub} = ['dataCompleteSLwjmFitInitp1000iterTolX01sub0' num2str(sub) '.mat']; 
end
...
parfor sub = 1 : length(subjects)
    ...
    fprintf('%s %i \n','(SLwjmPrefitInitpSub2to4onBowmore) Fitting subject ',sub)
    ...
    fprintf('%s \n','(SLwjmPrefitInitpSub2to4onBowmore) Fitting were successful...done')
...
fitP = fp;                %best fitp
tmpData = ['dataThisFunEvalSLwjmFitInitp1000iterTolX01sub0' num2str(sub)]; %save
