%SLwjmPrefitInitpSub2to4onSuze.m
%
...
%  usage:
%
%       o = SLwjmPrefitInitpSub2to7onSuze({'sub02','sub03','sub04'},[2 2 2 2.3 2.3 2.3 2.3 1 3],'experiment','vonMisesPrior') 
%    
%
...
function o = SLwjmPrefitInitpSub2to7onSuze(subjects,initp,varargin)
...
initp = repmat({initp},length(subjects),1);
...
parfor sub = 1 : length(subjects)
    ...
    fprintf('%s %i %s','(SLwjmPrefitInitpSub2to7onSuze) - subjects ',sub,' ...done\n')
...
for sub = 1 : length(subjects); 
    savedfNames{sub} =['dataCompleteSLwjmFitInitp1000iterTolX01sub0' num2str(sub)]; 
end
options = optimset('MaxIter',1000,'MaxFunEvals',10000,'TolFun',0.5,'TolX',0.1,'Display','iter');

parfor sub = 1 : length(subjects)
    
    [fitP{sub},neglogl(sub),exitflag{sub},outputfit{sub}] = fminsearch(@(fitP) SLgetLogL_WJM(data{sub},sCirad{sub},cCi{sub},kappaCi{sub},kappa_sCi{sub},prand{sub},km{sub},fitP{sub},posC{sub},sub),... 
        initp{sub},options);
    
    slparsave(savedfNames{sub}) %save
