%slFitWJMtoSub8onSherlock.m
%
%
% author: steeve laquitaine
%   date: 150821 updated 150821
%purpose: fit WJM model to motion direction estimation data
%
%  usage:
%
%       o = slFitWJMtoSub8onSherlock({'sub08'},[2 2 2 2.3 2.3 2.3 2.3 1e3 3e-30],'experiment','vonMisesPrior') 
%
%
%-----
%note: 
...
%
function o = slFitWJMtoSub8onSherlock(subjects,initp,varargin)
    ...
    %----------------------------------
    fprintf('%s \n','(slFitWJMtoSub8onSherlock) Gathering data ...')
    fprintf('%s \n','(slFitWJMtoSub8onSherlock) Please set dataPath ...')
    %dataPath = uigetdir(cd,'Pick a project e.g., /dataPsychophy/Exp01...');
    dataPath = '~/fitSub08';

    if ~isempty(dataPath)
        cd(dataPath)
        fprintf('(slFitWJMtoSub8onSherlock) Data path set to : \n')
        fprintf(['(slFitWJMtoSub8onSherlock) "',dataPath,'" \n'])
    else
        fprintf('(slFitWJMtoSub8onSherlock) Data path not set...aborting \n')
        keyboard
    end
    ...
    sub = 1;
    fprintf('(slFitWJMtoSub8onSherlock) Getting subject data...\n')
    ...
    fprintf('%s %s %s \n','(slFitWJMtoSub8onSherlock) - Database for subjects ',subjects{sub},' ...organized')
    ...
    %fit
    fprintf('%s %i \n','(slFitWJMtoSub8onSherlock) Fitting subject ',subjects{sub})
    ...
    fprintf('%s \n','(slFitWJMtoSub8onSherlock) Fitting was successful...done')
    tocs = toc;
