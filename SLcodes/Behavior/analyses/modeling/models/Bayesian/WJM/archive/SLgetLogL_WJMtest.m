...
%  usage: 
%
%       [loglAll,fitP,PestGivModel] = SLgetLogL_WJMtest(data,s,c,kappa,kappa_s,prand,km,fp,posC)
%
%
% outputs:
%
...
%get logl of data
function [loglAll,fitP,PestGivModel] = SLgetLogL_WJMtest(data,s,c,kappa,kappa_s,prand,km,fp,posC)
...
for Ci = 1 : nCon
    ...
    tic    
    %defined between -3.14 and 3.14
    x = circ_vmrnd(s(Ci)*ones(Ntrials,Ndots),kappa(Ci));
    toci(2,Ci) = toc ;

    %this is 50 ms faster than circ_vmrnd with preallocation
    %it is only .4s (instead of 4s) with parfor
    sde = round(SLra2d(s(Ci)))
    tic
    for i = 1 : 5000
        x(i,:) = randsample(-pi:2*pi/360:pi-(2*pi/360),328,true,vmPdfs(1:1:360,sde,kappa(Ci),'norm'));
    end
    toc
    %-------
    ...
    tic
    p2 = c(Ci)/2/pi/besseli(0,kappa(Ci),1);
    toci2(2,Ci) = toc;

    tic
    p3 = exp(kappa(Ci) * cos(bsxfun(@minus,x,svec))-kappa(Ci));
    toci2(3,Ci) = toc;
    
    tic
    p4 = bsxfun(@minus,x,svec);
    toci2(4,Ci) = toc;
