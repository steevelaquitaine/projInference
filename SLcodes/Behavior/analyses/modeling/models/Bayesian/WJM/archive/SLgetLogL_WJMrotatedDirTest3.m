%SLgetLogL_WJMrotatedDirTest3.m
%
%
...
%  usage: 
%
%       [loglAll,fitP,Pest] = SLgetLogL_WJMrotatedDirTest3(data,s,c,kappa,kappa_s,prand,km,fp,posC,Cond)
%
%
% outputs:
%
...
%get logl of data
function [loglAll,fitP,Pest] = SLgetLogL_WJMrotatedDirTest3(data,s,c,kappa,kappa_s,prand,km,fp,posC,Cond)
...
o24       = getPestEachCoh(Ntrials, Ndots, Ns, svec, su24, Nsu24 ,x1_24 ,rotrad24 ,coh(1),fp(1),prior80,prior40,prior20,prior10,prand,motn);%Pest coh 24%
o12       = getPestEachCoh(Ntrials, Ndots, Ns, svec, su12, Nsu12 ,x1_12 ,rotrad12 ,coh(2),fp(2),prior80,prior40,prior20,prior10,prand,motn);
o6        = getPestEachCoh(Ntrials, Ndots, Ns, svec, su6 , Nsu6  ,x1_6  ,rotrad6  ,coh(3),fp(3),prior80,prior40,prior20,prior10,prand,motn);

%estimates densities (Ncond by Nest)
...
[a,posNex]        = setdiff(single(mcond),single(Cond),'rows');  %non existing conditions out
modCond           = mcond;
modCond(posNex,:) = [];                 
posEx(posNex)     = [];
...
parfor i = 1 : Nsu
    ...
    like_per_dot = tail + amp*exp(kappa * cos(bsxfun(@minus,double(x),svec))-kappa);
