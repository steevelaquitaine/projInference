%SLgetLogL_WJMrotatedDir3Kappa.m
%
%
...
%  usage:
%
%       [loglAll,fitP,Pest] = SLgetLogL_WJMrotatedDir3Kappa(data,s,c,kappa,kappa_s,prand,km,fp,posC,Cond)
%
%
% outputs:
%
...
%get logl of data
function [neglogl,fitP,Pest] = SLgetLogL_WJMrotatedDir3Kappa(data,s,c,kappa,kappa_s,prand,km,fp,posC,Cond)
...
x1_12     = slCirc_vmrnd2(su12(1),fp(2),Ntrials,Ndots);
x1_6      = slCirc_vmrnd2(su6(1),fp(3),Ntrials,Ndots);
x1        = [{x1_24} {x1_12} {x1_6}];
motn      = (2/pi/besseli(0,km,1)*exp(km*cos(motevec)-km))'; %motor noise
motn      = motn/sum(motn);
prior80   = bsxfun(@plus,M,2/pi/besseli(0,fp(4),1)*exp(fp(4) * cos(svec - 3.93) - fp(4)));%Mean of prior is 225 deg (3.93 rads),need high precision
