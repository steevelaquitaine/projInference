...
for j = 1 : F.f2.n
    ...
    for i = 1 : F.f3.n
        ...
        %express -180º distance to prior as 180º (same but convenient
        %for visualization)
        ...
        %the prior mean (225º) and the x and y axes were centered 
        ...
        %on the circle (e.g., 0º, is labelled 225º). Those operations were 
        ...
        %estimate of 33º was calculated for 55º motion direction which is 
        ...
        %this visual artifact by expressing both 55º and 33º as the 
        %counterclockwise distance to prior mean (55º becomes 190º instead 
        %of 170º and 33º becomes 168º). Note that the maximum vectorial angle is  >180º. 
        if j==3 && ~isempty(intersect(xCenteredSorted,180))
            
            %move point at -170º distance to prior at 190º (positive side) 
            %and convert values at x=-170º to positive distance relative to prior
...
for j = 1 : F.f2.n
    ...
    for i = 1 : F.f3.n
        ...
        %express -180º distance to prior as 180º to conserve same format as
        %for estimates mean
        xTickCentered(xTickCentered==-180) = 180;
        ...
        %mean on a linear space. Thus y and x are sometimes > or < 180º which 
        %is the max distance to prior mean on a circular space (e.g., -170º move to 190º)
        if j==3 && ~isempty(intersect(xCenteredSorted,180))
...
%             'displayName','Bayes');
    ...
    %             'FontAngle','oblique',...
    %             'LineWidth',0.5,...
    %             'ytick',0 : 20 : max(max([yDataCenteredSorted+eDataCenteredSorted;...
    %             yCenteredSorted+ePredCenteredSorted])),...
    %             'yticklabel',0 : 20 : max(max([yDataCenteredSorted+eDataCenteredSorted;...
    %             yCenteredSorted+ePredCenteredSorted])))
