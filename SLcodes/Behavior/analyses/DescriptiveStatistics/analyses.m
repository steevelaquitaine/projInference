...
%     e.g., 1
%           SLinitAnalyses
%           analyses({'sub01'},{'StimStrength','Pstd','FeatureSample'},'experiment','vonMisesPrior','dataDis','signDistance');
%
%
%      %e.g. 2  
%           % run analyses with properly named data with
%           "slrenameFileForAnalysis.m" in the current path
%           analyses({'sub01'},{'StimStrength','Pstd','FeatureSample'},'inpath','experiment','vonMisesPrior','dataDis','signDistance');
%
%----------------
%Mandatory inputs
%----------------
...
%varargin :
%
%                    'inpath': run analyses on properly named data with
%                              "slrenameFileForAnalysis.m" in the current 
%                               path
...
%          'slgetprevDirEffect': test if data are biased toward the past trial
...
%Main analyses
%-------------
function [fig,GenStat,meanDispDirInfo,meanEstimateInfo,meanPrevDirInfo,FA1,FA2] = analyses(subjects,FAs,varargin)
...
if sum(strcmp(varargin,'dataPath'))
    ...
    %case data are in current directory
elseif any(strcmp(varargin,'inpath'))
    
    %Gather all available data in directory.
    dataPath = pwd;
    fprintf('%s \n','(analyses) Getting data in current directory...')
    databank = SLMakedatabank(subjects,varargin);
...
if sum(strcmp(varargin,'slgetprevDirEffect'))
...
%checking
if isempty(databank.data)
    fprintf('%s \n', 'Your databank is empty. Your path is probably wrong')
    dbstack; keyboard
end
...
for k=1:FA.g1.lvlsnb
    ...
    for j=1:FA.g2.lvlsnb
        ...
        %Linear fit
        linefit(FA.g3.lvlsnm',fig1.datamean.deg(j,:,k),FA.g3.lvlsnm',FA.g2.colorfit{j});
        ...
        if strcmp(type,'absDistance')
            ...
            for i=1:dist2Pr.uniqCount
                ...
                if i==dist2Pr.uniqCount
                    ...
                    set(gca,'xtick',5:20:355,'xticklabel',5:20:355)
                end
                xlim([0 400])
                ylim([0 max(pmax)])
                %drawPublishAxis
                        ...
                        for k=1:numel(ThedirlinDisttoPrior)
                            ...
                            for isub=1:length(sub)
                                ...
                                if ~isempty(dirtoHist)
                                    ...
                                    if k==numel(ThedirlinDisttoPrior)
                                        xlim([0 360])
                                        set(gca,'xtick',25:40:355,...
                                            'xticklabel',25:40:355)
            ...
            for j=1:numel(TheStimStrengths)
                ...
                for i=1:numPriors
                    ...
                    for k=1:numel(ThedirlinDisttoPrior)
                        ...
                        %no x-tick
                        set(gca,'xtick',25:40:355,'xtickLabel',[],'fontsize',10)
...
hold all;
for j=1:FA.g2.lvlsnb
    linefit(FA.g3.lvlsnm,fig3.datamean(:,j),FA.g3.lvlsnm',FA.g2.color{j});
end
